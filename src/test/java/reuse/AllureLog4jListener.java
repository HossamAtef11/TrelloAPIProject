    ...
    private void attachLogsToAllure() {
        ...
        if (logFile.exists() && logFile.length() > 0) {
            try (FileInputStream fileInputStream = new FileInputStream(logFile)) {
                ...
                Allure.addAttachment("Application Logs", "text/plain", fileInputStream, "log");
            } catch (IOException e) {
                log.error("Error attaching log to Allure report: ", e);
            }
        } else {
            log.warn("Log file not found or empty: " + logFile.getAbsolutePath());
        }
    }
}
